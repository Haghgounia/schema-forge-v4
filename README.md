...
Older phase/release documents remain under [`docs/`](docs/) as implementation history and validation evidence. The current official source checkpoint is `SCHEMAFORGE-V4-CONSOLIDATED-BASELINE-20260823-C8.10`, verified at 554 tests, 0 failures, 0 errors and 4 configuration-gated skips. C8 API/Application Service Decomposition is complete. C9 Test Matrix / Live-Validation Classification is also complete as a documentation-only checkpoint: all 189 test-source Java files are classified, including 149 default `*Test`, 34 opt-in `*IT` and 6 support files. C10 Documentation Consolidation is complete as well. See [`docs/testing/TEST-MATRIX-C9.md`](docs/testing/TEST-MATRIX-C9.md) and [`docs/reference/README.md`](docs/reference/README.md). Neither C9 nor C10 changes `src`; the frozen source fingerprint remains `03d01cfd60e6b04be02ecc8df9c0dd6c47d6f06dc07f77f6c60844a93d5102ba`. C11 Final Consolidation Regression / Baseline Freeze is now the active source-unchanged verification candidate.
...
scripts/<platform>/
reports/
manifest.json                  # Standard Manifest V1 for Word/Legacy/ZIP/EA
```
...
The REST endpoint `POST /api/v1/generate/ea-xml` accepts Enterprise Architect XML/XMI 1.x database-model exports. The multipart request accepts `file` and an optional `schema` parameter; an explicit API schema overrides schema/owner values embedded in EA and the configured fallback. Integer-compatible primary-key columns may be inferred as identity when the EA import policy enables PK identity inference; character/date primary keys are never made identity solely because they are primary keys. EA tables and columns are converted to the same canonical model used by Word input. Because one EA export may contain many tables, the response ZIP follows the common C5 artifact-first layout: one DDL file per table and dialect, comparison workbooks when metadata is available, one timestamped canonical `*.schema.json`, dialect-specific run-all scripts, diagrams/reports as applicable, and the Standard Manifest V1 `manifest.json`. C6.2 applies the same manifest contract to Word, Legacy Word, ZIP Batch, and EA.

All four artifact-generation REST endpoints (`word`, `legacy-word`, `zip`, `ea-xml`) also accept request-level audit options: `includeAuditFields=true|false` and `auditProfile=AUTO|CREATED_UPDATED|CREATED_LAST_MODIFIED`. `AUTO` detects the audit family already present in the source model and completes only that family. Source-defined audit columns are preserved exactly; SchemaForge never replaces their datatype, nullability, default, description, identity, or physical options merely to match an audit profile. A table that mixes both supported audit families fails with `AUDIT_PROFILE_CONFLICT` instead of receiving a six/seven-column mixed audit set.
...
R7.2 cross-dialect recovered-corpus acceptance: `docs/R7.2-FIVE-DBMS-CORPUS-ACCEPTANCE.md`.
