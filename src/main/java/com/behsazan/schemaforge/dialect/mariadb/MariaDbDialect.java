...
/**
 * MariaDB logical DDL dialect.
 *
 * <p>MariaDB participates in the logical and physical SchemaForge contracts. Physical
...
public final class MariaDbDialect implements Dialect {
    ...
    private static final int MARIADB_MAX_LOGICAL_ROW_BYTES = 65_535;
    private static final int MARIADB_TEXT_MAX_BYTES = 65_535;
    private static final int MARIADB_MEDIUMTEXT_MAX_BYTES = 16_777_215;
    private static final int OFF_ROW_POINTER_BUDGET_BYTES = 12;
    private static final int VARCHAR_LENGTH_PREFIX_BYTES = 2;
    private static final int MARIADB_TSTZ_TEXT_LENGTH = 128;
    private static final int MAX_UTF8MB4_VARCHAR_CHARACTERS =
            (MARIADB_MAX_LOGICAL_ROW_BYTES - VARCHAR_LENGTH_PREFIX_BYTES) / UTF8MB4_MAX_BYTES_PER_CHARACTER;
    ...
    private final MariaDbTypeMapper typeMapper;
    private final MariaDbIdentifierRenderer identifierRenderer = new MariaDbIdentifierRenderer();
    private final MariaDbExpressionMapper expressionMapper = new MariaDbExpressionMapper();

    public MariaDbDialect() {
        this(NumericMappingStrategy.SAFE);
    }

    public MariaDbDialect(NumericMappingStrategy numericMappingStrategy) {
        this.numericMappingStrategy = Objects.requireNonNull(
                numericMappingStrategy, "numericMappingStrategy must not be null");
        this.typeMapper = new MariaDbTypeMapper(numericMappingStrategy);
    }
    ...
    public String sqlType(Column column) {
        ...
        String mapped = isTimestampWithTimeZone(column.dataType())
                ? "VARCHAR(" + MARIADB_TSTZ_TEXT_LENGTH + ")"
        ...
        throw new IllegalArgumentException(
                "MariaDB AUTO_INCREMENT requires an integer column; no lossless integer mapping exists for "
    ...
    public String inlineColumnConstraintClause(Table table, Column column) {
        ...
        if (textStoragePromotions(table).contains(column.name().normalized())) {
            ...
            clause.append(" CHECK (CHAR_LENGTH(").append(quote(column.name())).append(") <= ")
                    ...
                    .append(" /* SchemaForge MariaDB storage adaptation: ").append(source)
        ...
        if (isTimestampWithTimeZone(column.dataType())) {
            clause.append(" /* SchemaForge MariaDB portability adaptation [MARIADB-TSTZ-TEXT-001]: ")
                    .append("TIMESTAMP WITH TIME ZONE -> VARCHAR(").append(MARIADB_TSTZ_TEXT_LENGTH)
                    .append("); MariaDB TIMESTAMP normalizes through the session time zone and does not ")
    ...
    public String inlineColumnConstraintClause(
        ...
        if (column.identity()) {
            return existing
                    + " /* SchemaForge MariaDB portability adaptation: NUMBER(19,0) identity -> "
                    + "BIGINT UNSIGNED; AUTO_INCREMENT values are nonnegative; review pre-existing "
                    + "negative source values before migration */";
        }
        return existing
                + " /* SchemaForge MariaDB FK type adaptation: NUMBER(19,0) -> BIGINT UNSIGNED "
    ...
    public void validateTable(Table table) {
        ...
        if (identities.size() > 1) {
            throw new IllegalArgumentException(
                    "MariaDB permits only one AUTO_INCREMENT column per table: " + table.qualifiedName());
        }
    ...
    public List<String> supplementalCreateTableDefinitions(Table table) {
        ...
        return List.of(
                "  KEY " + quote(Identifier.of(indexName)) + " (" + quote(identity.name()) + ")"
                        + " /* SchemaForge MariaDB portability adaptation [MARIADB-AUTO-INDEX-001]: "
    ...
    public boolean emitSequence(DatabaseSchema schema, Sequence sequence) {
        ...
        // MariaDB implements that same identity intent with AUTO_INCREMENT, so this parser-
    ...
    public String defaultClause(Column column) {
        ...
        // MariaDB implements the logical identity directly and must not emit a DEFAULT here.
        if (column.identity()) {
            return identityClause(column);
        }
        String sourceExpression = column.defaultValue().expression();
        // MariaDB rejects an explicit DEFAULT NULL on a NOT NULL column (ER_INVALID_DEFAULT).
        ...
        String mapped = expression(sourceExpression);
        if (mapped.regionMatches(true, 0, "NEXT VALUE FOR ", 0, "NEXT VALUE FOR ".length())) {
    ...
    public String qualifyIndexName(QualifiedName tableName, String renderedIndexName) {
        // MariaDB index names belong to the table namespace and are not schema-qualified.
    ...
    public String indexTablespaceClause(String tablespace) {
        // MariaDB has table/general-tablespace placement, not an independent CREATE INDEX TABLESPACE clause.
        return "";
    }
    ...
    public String referentialActionClause(String clause, ReferentialAction action) {
        if (action == ReferentialAction.SET_DEFAULT) {
            throw new UnsupportedOperationException(
                    "MariaDB does not emit SET DEFAULT referential actions");
        }
    ...
    public String schemaBootstrapStatement(Identifier schemaName) {
        Objects.requireNonNull(schemaName, "schemaName must not be null");
        // In MariaDB, SCHEMA is a synonym for DATABASE. Qualified table names therefore use
    ...
    private Set<String> textStoragePromotions(Table table) {
        ...
        if (remainingVarcharBytes <= MARIADB_MAX_LOGICAL_ROW_BYTES) {
        ...
        if (remainingVarcharBytes > MARIADB_MAX_LOGICAL_ROW_BYTES) {
            throw new IllegalArgumentException(
                    "MariaDB row-size adaptation cannot preserve indexed/key/defaulted VARCHAR semantics for "
    ...
    private String promotedTextType(Column column) {
        ...
        if (maximumBytes <= MARIADB_TEXT_MAX_BYTES) {
            return "TEXT";
        }
        if (maximumBytes <= MARIADB_MEDIUMTEXT_MAX_BYTES) {
            return "MEDIUMTEXT";
        }
        return "LONGTEXT";
    }
    ...
    private void requireTextPromotionEligible(Table table, Column column, String reason) {
        if (!textPromotionEligible(table, column)) {
            throw new IllegalArgumentException(
                    "MariaDB requires off-row text storage for " + table.qualifiedName() + "."
    ...
    private String losslessIdentityInteger(DataType type) {
        ...
        // MariaDB AUTO_INCREMENT generates nonnegative values. BIGINT UNSIGNED covers the full
        // nonnegative NUMBER(19,0) range while retaining an integer type required by MariaDB.
        return "BIGINT UNSIGNED";
    }
    ...
    private String autoIncrementSupportIndexName(Table table, Identifier column) {
        ...
        String candidate = truncateMariaDbIdentifier(base, "");
        int suffix = 2;
        while (occupied.contains(candidate.toUpperCase(Locale.ROOT))) {
            String suffixText = "_" + suffix++;
            candidate = truncateMariaDbIdentifier(base, suffixText);
        }
        return candidate;
    }

    private String truncateMariaDbIdentifier(String base, String suffix) {
        int maximum = 64;
