 ...
 * are mandatory. Blocked, skipped, or failed outcomes may omit them.</p>
 *
 ...
 *
 * <p>{@code outcomeReason} is optional for generated artifacts, mandatory for blocked artifacts,
 * and captures a human-readable, deterministic reason for blocked/skipped/failed outcomes so
 * package consumers do not need to infer the reason from logs or a separate report.</p>
 */
public record ArtifactDescriptor(
    ...
    public ArtifactDescriptor {
        ...
        if (!mediaType.isEmpty() && !mediaType.contains("/")) {
            throw new IllegalArgumentException(
                    "mediaType must be a valid type/subtype value");
        }
        ...
        if (status == ArtifactStatus.GENERATED) {
            ...
            if (relativePath.isEmpty()) {
                throw new IllegalArgumentException(
                        "relativePath is required for generated artifacts");
            }

            if (mediaType.isEmpty()) {
                throw new IllegalArgumentException(
                        "mediaType is required for generated artifacts");
            }
        }

        if (status == ArtifactStatus.BLOCKED && outcomeReason.isEmpty()) {
            throw new IllegalArgumentException(
    ...
    /**
     * True for artifacts such as canonical JSON, reports, and diagrams
     * that are DBMS-neutral.
     */
    ...
    private static void validateRelativePath(String path) {
        ...
        if (path.startsWith("/")
            ...
            throw new IllegalArgumentException(
                    "relativePath must not be absolute: " + path);
        }

        if (path.indexOf('\\') >= 0) {
            throw new IllegalArgumentException(
                    "relativePath must use '/' separators: " + path);
        }
        ...
        for (String segment : segments) {
            ...
            if (segment.isEmpty()
                ...
                throw new IllegalArgumentException(
                        "relativePath contains an invalid segment: " + path);
            }
        }
    }
}